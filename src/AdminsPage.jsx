...
const filtered = data.filter(admin => {
    ...
    const handleAddAdmin = () => {
        ...
        .then(res => res.json())
        .then(() => { alert('Admin added successfully'); fetchAdmins(); setShowModal(false); resetForm(); })
    ...
    const handleEditAdmin = () => {
        ...
        .then(res => res.json())
        .then(() => { alert('Admin updated successfully'); fetchAdmins(); setShowModal(false); resetForm(); })
        .catch(() => alert('Failed to update admin'));
    };

    const handleDeleteAdmin = (adminId) => {
        if (window.confirm('Are you sure you want to delete this admin?')) {
            fetch(`https://taxiwheel-backend.onrender.com/admins/${adminId}`, { method: 'DELETE' })
            .then(res => res.json())
            .then(() => { alert('Admin deleted successfully'); fetchAdmins(); })
            .catch(() => alert('Failed to delete admin'));
        }
    };

    const resetForm = () => {
        setAdminForm({ firstName: '', lastName: '', email: '', password: '', phoneNo: '' });
    ...
    const openEditModal = (admin) => {
        ...
        setAdminForm({
            ...
            email: admin.Email, password: admin.Password, phoneNo: admin.PhoneNo
        });
        setShowModal(true);
    };
    ...
    return (
        <div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" style={{zIndex: 50}}>
                    <div className="bg-white p-8 rounded-lg w-96">
                        ...
                        <div className="space-y-4">
                            {[['firstName','First Name'],['lastName','Last Name'],['email','Email'],['password','Password'],['phoneNo','Phone Number']].map(([name, placeholder]) => (
                                <input key={name} type="text" name={name} placeholder={placeholder}
                                    value={adminForm[name]} onChange={handleInputChange}
                                    className="w-full p-2 border rounded" />
                            ))}
                            <div className="flex justify-between">
                                <button onClick={() => setShowModal(false)} className="bg-gray-300 text-black px-4 py-2 rounded">Cancel</button>
                                <button onClick={modalMode === 'add' ? handleAddAdmin : handleEditAdmin} className="bg-yellow-400 text-white px-4 py-2 rounded">
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={openAddModal} className="bg-yellow-400 text-white px-4 py-2 rounded">Add Admin</button>
            </div>
            ...
            <div className="bg-white rounded-lg shadow">
                <table className="w-full">
                    ...
                    <tbody>
                        {filtered.length === 0 ? (
                            <tr><td colSpan="7" className="p-4 text-center text-gray-500">No admins found</td></tr>
                        ) : filtered.map((admin) => (
                            <tr key={admin.Adminid} className="border-b">
                                ...
                                <td className="p-4 flex gap-2">
                                    <Eye onClick={() => handleViewAdmin(admin)} className="w-5 h-5 text-blue-500 cursor-pointer" />
                                    <Edit onClick={() => openEditModal(admin)} className="w-5 h-5 text-yellow-500 cursor-pointer" />
                                    <Trash onClick={() => handleDeleteAdmin(admin.Adminid)} className="w-5 h-5 text-red-500 cursor-pointer" />
                                </td>
                            </tr>
                        ))}
                    </tbody>
